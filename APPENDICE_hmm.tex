...
La seguente appendice sulle HMM fornisce una spiegazione nel dettaglio necessario al chiarimento dell'uso che ne viene fatto nel lavoro. Come spiegato nel Capitolo \ref{sec:modelloHmm}, le HMM sono usate per modellare la deambulazione e l'algoritmo di Viterbi è usato per la segmentazione della deambulazione.\\
Le HMM sono strumenti di modellazione di sequenze temporali. Un comune esempio di applicazione è il riconoscimento della comunicazione verbale (\textit{Speech Recognition}) \cite{tutorial_hmm_application_speech_recognition}. 
...
I processi di Markov possono essere raggruppati in base al tempo che può essere discreto o continuo, ed allo spazio degli stati che può essere anche esso discreto e finito o continuo. 
...
\begin{equation}
\wp [q_t = s_{i0} | q_{t-1} = s_{i1}, s_{i2}, \ldots, q_0 = s_{it}]
\label{eq:fullProbDescription}
\end{equation}
Ovvero la probabilità che il processo di Markov si trovi nello stato $s_{i0}$ a tempo $t$ dato che a tempo $t-1$ si trovava nello stato $s_{i1}$, a tempo $t-2$ nello stato $s_{i2},$ e nello stato iniziale $q_0$ era nello stato $s_{it}$.
Un caso speciale di \eqref{eq:fullProbDescription} in cui la probabilità che il sistema si trovi in un determinato stato nel presente dipende solo dall'istante di tempo precedente e non da tutta la storia a partire dal primo istante di tempo corrisponde alla Definizione \ref{eq:first_Order_MM}.
...
\begin{equation}
\begin{split}
	\pi   &= (pi_i)\quad 1\leq i \leq N \quad \text{dove}\\ 
	\pi_i &= \wp(q_1 = S_i)\\
\end{split}	
\label{eq:prior}
\end{equation}
...
\begin{equation}
\begin{split}
B = &\{b_j(k)\}\quad \text{dove} 1\leq j \leq N \quad , 1\leq k \leq M \\
    &b_j(k) = \wp[v_k \quad \text{all'istante } t | q_t = s_j] 
\end{split}	 
...
Una HMM può essere usata per generare una sequenza di osservazioni $O=\{O_1,O_2,\ldots,O_T\}$ nel seguente modo:
\begin{algorithm}    
...
\begin{figure}
	...
	\caption{Esempi di distribuzioni gaussiane monovariate. Figura adattata da Wikipedia.}
	\label{fig:monovargaussian}
\end{figure}

\begin{figure}
	\centering
		\includegraphics[width=.8\textwidth]{imgs/bivariategaussian.jpg}
	\caption{Esempio di distribuzioni gaussiane multivariate con due variabili in ingresso: distribuzione bivariata. Figura adattata da Wikipedia.}
	\label{fig:bivargaussian}
\end{figure}

\begin{figure}
	\centering
		\includegraphics[width=.8\textwidth]{imgs/bivariategaussianmixture.jpg}
	\caption{Esempio di una combinazione di due distribuzioni gaussiane bivariate. Figura adattata da Wikipedia.}
	\label{fig:mixbivargaussian}
\end{figure}

La formula \ref{eq:continuousEmissionMtx} può essere generalizzata su due fronti, il primo è il numero di variabili in ingresso che può essere $n$. Una distribuzione gaussiana con $n$ variabili in ingresso è detta distribuzione gaussiana mutivariata (vedi Figura \ref{fig:bivargaussian}). La seconda generalizzazione che è possibile apportare alla formula \ref{eq:continuousEmissionMtx} è combinare, mediante ad esempio una combinazione lineare, più distribuzioni in una unica funzione (vedi Figura \ref{fig:gaussianmixture}).
...
\begin{equation}
\begin{split}
B = &(b_j(x))\quad \text{dove}\\
		& b_j(x) = \displaystyle \sum_{m = 1}^{M} c_{j,m}\mathcal{N}(x,\bm\mu_j,\bm\Sigma_j)\quad \text{ per } 1 \leq j \leq N
\end{split}
\label{eq:continuousEmissionMtxGauusianMixtures}
\end{equation}

Dalla definizione \ref{eq:continuousEmissionMtxGauusianMixtures} segue 
\begin{equation}
...
\section*{Tipi di problemi che si affrontano con le HMM}

Generalmente con le HMM si affrontano 3 tipologie di problemi. 

\begin{enumerate}
	\item Valutazione: dati $O$ e $\lambda$, calcolare $\wp(O|\lambda)$ in modo ottimale.
	\item Decodifica: dati $O$ e $\lambda$, trovare la sequenza di stati $Q=q_1,q_2,\ldots, q_T$ ``migliore'', secondo un criterio di ottimalità, che possa aver generato $O$.
	\item Apprendimento: dato $\lambda$, regolarne i parametri per massimizzare $\wp(O|\lambda)$?
\end{enumerate}
...
La soluzione ottima a questo problema è data dall'algoritmo di programmazione lineare noto come algoritmo \textit{Forward}. L'idea su cui si basa è quella di considerare dei percorsi parziali per rappresentare osservazioni parziali. Vengono definite le variabili $\alpha_{i,t}$ come la probabilità di aver osservato la sequenza parziale $O_1, \ldots, O_t$ e di essere nello stato $S_i$ all' istante temporale $t$ 
\begin{equation}
\alpha_{i,t} = \wp(o_1, \ldots, o_t, q_T=S_i|\lambda)
\label{eq:alpha}
\end{equation}

Dalla definizione \ref{eq:alpha} ricaviamo la matrice $\alpha (N \times T)$ nella quale vengono inserite le probabilità parziali per ogni combinazione stato-tempo, di modo che al passo temporale successivo il calcolo possa essere basato su tali valori e non ricalcolando tutto dal primo istante di tempo. 
\begin{algorithm}    
...
\label{eq:gamma}
\end{equation}
La variabile $\gamma$ può essere calcolata in funzione delle variabili di \textit{Forward} e \textit{Backward}:
...
Il valore $\alpha_{i,t}$, fornisce le probabilità delle osservazioni parziali fino a $t$, mentre $\beta_{i,t}$, da $t+1$ in poi, essendo correntemente nello stato $S_i$. Il denominatore dell'equazione \ref{eq:gammaFnAlphaBeta} è un fattore di normalizzazione che rende $\gamma$ un valore di probabilità, quindi sarà valida la proprietà
\begin{equation}
...
Il criterio di ottimalità può essere cambiato per individuare la miglior sequenza di lunghezza $T$ che massimizzi la probabilità di tutti gli stati nella sequenza. Per trovare il cammino migliore, viene usato un metodo di programmazione dinamica detto algoritmo di Viterbi.
...
La procedura completa per trovare il percorso di stati più probabile è descritta nell'Algoritmo \ref{alg:viterbi}.
\begin{algorithm} 
\caption{Viterbi}
\label{alg:viterbi}                                                  
\begin{algorithmic}                    
...
Si definisce punto di fusione, l'istante temporale $\tau < T$  t.c per $a \leq t \leq \tau$, tutti i cammini locali appartenenti all'insieme dei cammini locali $CL =\{ s(a,b,i), \forall i \in S\}$ sono uguali.
Per $S$ si intende l'insieme degli stati dell'HMM.
\end{definition}
	
Un punto di fusione gode della seguente proprietà:
i cammini locali fino al punto di fusione (che per definizione sono tutti uguali) sono sempre uguali al cammino globale (quello ottenibile con l'Algoritmo di Viterbi originale \ref{alg:viterbi})\footnote{Per la dimostrazione consultare \cite{short_time_viterbi_online_hmm_deconding}}.
...
Dato il modello $\lambda = (A,B,\pi)$ indico il modello estimato con $\overline{\lambda} = (\overline{A},\overline{B},\overline{\pi})$. L.Baum et al\cite{maximization_technique_stat_analysis_fun_markov_ch} hanno dimostrato che nel processo di stima è vera una delle seguenti alternative:
\begin{enumerate}
...
\begin{itemize}
	...
	\begin{equation}
			c_t = \frac{1}{\displaystyle\sum_{j=1}^{N}\alpha_{i,t}}
	...
	\end{equation}
	dove $\epsilon$ rappresenta un peso che viene associato ai parametri del modello, ed $(1- \epsilon)$ il peso associato a quelli del modello ridotto. Il valore di $\epsilon$ viene determinato in funzione dei dati di addestramento. Mercer et al \cite{Mercer_interpolation_markov_params_sparse_data} hanno dimostrato che è possibile stimare l'$\epsilon$ ottimo mediante l'algoritmo \textit{Forward}-\textit{Backward}, espandendo la HMM a partire da \ref{eq:modelInterpolation}.
