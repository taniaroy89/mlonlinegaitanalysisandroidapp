...
\label{cap:hmm}
I HMM sono strumenti di modellazione di sequenze temporali. Un comune esempio di applicazione è il riconoscimento della comunicazione verbale (Speech Recognition)\cite{tutorial_hmm_application_speech_recognition}. 
Una sequenza temporale non è altro che un segnale.

\begin{definition}[Segnale]
	Un segnale è l'\OUTPUT osservabile di un processo,
...
\section{Processi di Markov Discreti}
La tipologia più semplice di modello di Markov sono quelli discreti.

\begin{definition}[Processi di Markov Discreti]$<N,M,A,\pi>$\\
Dato un insieme di stati $S = \{s_1,\ldots, s_N\}$ sono detti Processi di Markov Discreti dei 
sistemi che ad ogni istante di tempo $t$ assumono uno stato, ovvero lo stato corrente $q_t = s_k \in S$. \\
\begin{itemize}
	\item $q_0 = \displaystyle \max_{0 \leq i \leq N} \pi_i$ 
	\item $q_t = \displaystyle \max_{0 \leq j \leq N} A_{q_{t-1},j}\quad  1 \leq t \leq T$
\end{itemize}
\end{definition}
...
\begin{figure}
	\centering
		\includegraphics[width=1.00\textwidth]{imgs/descreteMarkovProcesses.jpg}
	\caption{Rappresentazione a lattice di Processo Discreto di Markov}
...
\begin{equation}
\wp [q_t = s_j | q_{t-1} = s_i, \ldots, q_0 = S_k]
...
dove:
 \begin{enumerate}
	...
	\item $B$ è la matrice di probabilità dei Simboli di Osservazione definita in \eqref{eq:emissionMtx},
	\item $\pi$ è il vettore di probabilità a priori definto in \eqref{eq:prior}.
\end{enumerate}
\end{definition}

Un HMM può essere usato per generare una sequenza di osservazioni $O=\{O_1,O_2,\ldots,O_T\}$ nel seguente modo:
\begin{algorithm}    
...
Il problema della valutazione consiste nel calcolare la probabilità che una certa sequenza di osservazioni sia stata prodotta da un dato modello.
...
Una soluzione naïve della Valutazione è quello di enumerare tutte le possibili sequenze di stati (detti anche percorsi)  $Q = q_1, \ldots, q_T$ dove $q_i \in S={s_1,s_2,\ldots, s_N}$ e calcolare per ciascuna la probabilità che l'osservazione sia stata prodotta da essa e moltiplicare il risultato per la probabilità che quel particolare percorso venga scelto.
\begin{equation}
\begin{split}
...
Nella decodifica, si cerca la sequenza di stati $Q = q_1,\ldots,q_T$ che ha generato una data sequenza di osservazioni $O = o_1,\ldots, o_T$. Dato che, al contrario della Valutazione, non esiste un'unica soluzione al problema delle Decodifica, quello che si fa è di stabilire un criterio di ottimalità in funzione del quale fare la ricerca. \\
...
Anche se \eqref{eq:statoPiùProbabile} massimizza il numero di stati più probabili scegliendo quelli che individualemente sono i più probabili, potrebbe creare problemi nel momento in cui si considera una sequenza di stati. Se la HMM ha anche transizioni nulle, la sequenza generata da \eqref{eq:gammaFnAlphaBeta} potrebbe essere non valida, perché non vi è nessun controllo sulla probabilità di co-occorrenza di stati. \\
Il criterio di ottimlalità può essere cambiato ad individuare la miglior sequenza di lunghezza $T$ che massimizzi la probabilità di tutti gli stati nella sequenza. Per trovare il cammino migliore, viene usato un metodo di programmazione dinamica detto algoritmo di Viterbi.
...
i cammini locai fino al punto di fusione (che per definizione sono tutti uguali) sono sempre uguali al cammino globale (quello ottenibile con l'algortimo di Viterbi originale \ref{alg:viterbi})\footnote{per la dimostrazione consultare \cite{short_time_viterbi_online_hmm_deconding}}.
...
\end{itemize}
La procedura iterativa è detta riestimazione, e consiste in un miglioramento graduale (in base ad un criterio) ed un aggiornamento dei parametri. Per descivere le riestimazione, è necessario definire la variabile $\xi$ come la probabilità di essere in un certo stato in un istante di tempo ed essere in un altro nel successivo istante di tempo:
\begin{equation}
\begin{split}
...
Nel secondo caso è possibile sostituire $\overline{\lambda}$ a $\lambda$, ripetendo l'iterazione, finché non si verifica una condizione d'arresto, ed il risultato della procedura è detto stima di massiva verosimiglianza (maximum likelihood estimate) dell'HMM
\begin{algorithm}    
\caption{Baum-Welsh o Maximum Likelihood Expectation.  }
...
\begin{itemize}
	...
	La modifica più importante deve essere applicata all'algoritmo Forward, perché si è interessati al valore di probabilità. In questo caso non è possibile semplicemente sommare $\hat{\alpha_{j,t}}$, perché sono valori scalati e privi di significato se presi singolarmente. In questo caso viene usata la seguente proprietà:
	\begin{equation}
		\begin{split}
	...
	\end{equation}
	dove $\epsilon$ rappresenta un peso che viene associato ai parametri del modello, ed $(1- \epsilon)$ il peso associato a quelli del modello ridotto. Il valore di $\epsilon$ viene determinato in funzione dei dati di addestramento. Mercer et al \cite{interpolation_markov_params_sparse_data} hanno dimostrato che è possibile stimare l'$\epsilon$ ottimo mediante l'algoritmo Forward-Backward, espandendo la HMM a partire da \eqref{eq:modelInterpolation}.
