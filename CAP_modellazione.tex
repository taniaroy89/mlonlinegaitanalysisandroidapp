...
\section{Modello: HMM}
Il modello di deambulazione scelto è una HMM minimale a 4 stati sinistra-destra ad emissioni continue (vedi appendice \ref{sec:tipi_hmm}). Tale scelta è stata fatta per poter modellare la deambulazione con il minimo numero di parametri possibile. \TODO{Inoltre il segnale del giroscopico è suddivisibile anche ad occhio in 4 stati?}
...
\begin{equation}
	a_{ij} > 0 \Leftrightarrow j = i+1 \quad \text{o} \quad j = i+2 \quad \text{o} \quad(i = N \quad\text{e}\quad j = 1)
\label{eq:}
\end{equation}
...
Le regole riportate nelle tabelle \ref{tab:vicon_rules, tab:min_max_HM_TM} e figura \ref{fig:vicon_traced_TM_HM} usate per generare il segnale di riferimento Vicon\tm si basano sul tracciamento di soli 2 marcatori: HM e TM.
...
In particolare poiché sul \textit{dataset} i dati sono amobili e conosciamo quindi la fase del cammino cui appartengono, è possibile procedere all'addestramento con approccio supervisionato. L'associazione di ogni stato del modello ad
...
La validazione è stata fatta con l'approccio della validazione incrociata ad esclusione di un campione (\textit{leave-one-subject-out cross-validation} o LOOCV). 
Il metodo consiste nell'addestrare un modello su P-1 ($P=6$) soggetti e testarne la validità su 1 soggetto. La procedura viene ripetuta P volte in modo da verificare ogni soggetto su un modello addestrato usando i dati usando i dati degli altri 5. I risultati di tutte le P validazioni eseguite vengono in fine aggregati in un unico risultato.
...
\section{Problematiche implementative e soluzioni}
Il modello sinistra-destra dell'HMM (vedi appendice \ref{sec:HMM}) può portare a considerare cicli di deambulazione aggiuntivi detti inserzioni (\textit{insertions}), o meno di quelli effettivi delezioni (\textit{deletions}). Il problema delle inserzioni viene risolto mediante l'applicazione di un'euristica: per i presupposti del tipo di deambulazione considerata (velocità e pendenza del terreno), tutti i cicli di deambulazione di durata inferiore a 250 ms secondi sono inserzioni.
