...
La seconda parte del lavoro è relativa allo sviluppo di un'applicazione per uno Smartphone Android\tm che segmenta in linea il cammino, a partire da dati giroscopici (vedi fig \ref{fig:schema_app}).
\begin{figure}[h]
	\centering
...
Dato che il nucleo (\textit{kernel}) di Android-OS è basato su Linux (vedi Appendice \ref{app:android}) usa una politica di controllo di accessi (\textit{Access Control})\footnote{Accesso controllato dell'utente di un \textit{filesystem} o di un servizio} molto simile a quello del sistema operativo Linux. Tutti i servizi che non sono forniti da un programma diverso da quello in uso possono essere utilizzati solo se l'utente dà il permesso di utilizzarli, vale a dire che un programma non può interagire con altri programmi se non con l'esplicita autorizzazione dell'utente. Per questo motivo l'applicazione deve elencare tutte le richieste di permessi di cui necessita. L'elenco deve essere fatto nella porzione iniziale dell'\verb|AndroidManifest|.\\
 ...
 In dettaglio una API nei linguaggi orientati ad oggetti, sono concepiti come l'insieme di tutti i metodi pubblici che le classi pubbliche offrono.} di Android\tm e per interagire con altri applicazioni.
...
La parte seguente della schermata, a partire dall'alto, è un altro \verb|LineraLayout| contenente campi di testo e pulsanti.\\
...
Abbiamo ritenuto utile avere un protocollo di comunicazione fra \textit{thread} mediante una struttura che abbiamo chiamato "\textit{mailbox}", in cui $n$ \textit{thread} possano inserire e consumare dati.\\
\subsubsection{MailBox}
\label{sec:mailbox}
L'oggetto \verb|MailBox| è un \textit{buffer} di tipo generico, a cui si può accedere mediante un inserimento di tre modalità: sincronizzata, temporizzata e condizionata. 
\begin{enumerate}
	\item \textbf{Sincronizzata}: l'inserimento e prelievo di dati dalla \textit{mailbox} è bloccante, un \textit{thread} inserisce un dato se c'è spazio nel buffer, un altro \textit{thread} utilizza il dato, se esiste e lo rimuove dal buffer. Ciascuno attende il proprio turno per compiere l'azione. La sincronizzazione viene gestita con un meccanismo di semafori che Java\tm fornisce.
